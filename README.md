...
| `/grok:fan-out <task>` | Fans a task out across several **parallel** Grok calls — one per expert angle (default: researcher, reviewer, security-auditor, test-writer) — then a synthesis call that reconciles them into one consolidated report (a section per angle + a `## Consolidated verdict`). N+1 real Grok calls, much deeper than a plain `/grok:ask`. Read-only by default; `--write` is gated. Pick angles with `--personas`, or name custom angles with `--agents-json`. |
